...
/*   Updated: 2021/09/07 14:08:01 by javgonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_extra_variable(char *new_var)
{
	...
	g_minishell_data.extra_variables = malloc(sizeof(*g_minishell_data.extra_variables) * (g_minishell_data.extra_variables_size + 2));
	i = 0;
	while (old_vars[i].name != NULL)
	{
		g_minishell_data.extra_variables[i].name = ft_strdup(old_vars[i].name);
		g_minishell_data.extra_variables[i].value = ft_strdup(old_vars[i].value);
		i++;
	}
